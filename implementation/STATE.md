...
| **Current task** | _phase code-complete_ — 1.1, 1.2, 1.3 all `done-pending-review`; next is review + PR |
| **Tasks verified** | 0 / 58 |
| **Phases merged** | 0 / 16 |
...
| B3 | ~~`sentinel-gha` UAMI + 2 role assignments + first 2 federated credentials, then the 5-object import~~ | — | Keshav | ✅ **CLOSED 2026-08-15.** UAMI `sentinel-gha` (`clientId c9ed809b-eca3-4ecc-8678-5dbfb91be5ae`), Contributor on `sentinel-rg`, Storage Blob Data Contributor on `sentineltfstate0375`, **all 5 federated credentials** live. Five objects imported, three created by apply, `terraform plan` → **"No changes"**. rev-9 proven end to end: a managed identity does substitute for an app registration. |
